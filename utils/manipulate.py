...
def SIMGAN_generate(Gs,Zs,reals,NoiseAmp, Gs2,Zs2,reals2,NoiseAmp2,Gs3, Zs3, reals3, NoiseAmp3,  opt,in_s=None,gen_start_scale=0):
    ...
    for G,G2,Z_opt,real_curr,real_next,noise_amp in zip(Gs,Gs2,Zs,reals,reals[1:],NoiseAmp):
        ...
        x_ac = x_ac[:, :, 0:real_curr.shape[2], 0:real_curr.shape[3]]
        z_in = noise_amp * z + real_curr
        x_ac = G(z_in.detach(), x_ac, [1, 0])
        x_aca = G2(x_ac, x_aca)
        x_ac = imresize(x_ac.detach(), 1 / opt.scale_factor, opt)
        x_ac = x_ac[:, :, 0:real_next.shape[2], 0:real_next.shape[3]]
        x_aca = imresize(x_aca.detach(), 1 / opt.scale_factor, opt)
        x_aca = x_aca[:, :, 0:real_next.shape[2], 0:real_next.shape[3]]
        count += 1
        plt.imsave('%s/x_ac_%d.png' % (dir2save,count), functions.convert_image_np(x_ac.detach()), vmin=0,vmax=1)
    return x_ab.detach(), x_ac.detach()
